...
void mpFXYVector::AddDataScrolling(float x, float y, std::vector<double> &xs, std::vector<double> &ys, float deltaX)
{
    // Check if the data vectora are of the same size
    if (xs.size() != ys.size())
	{
...
void mpWindow::FitY(double yMin, double yMax, wxCoord *printSizeX,wxCoord *printSizeY)
{
	// Save desired borders:
	m_desiredXmin=m_minX; m_desiredXmax=m_maxX;
	m_desiredYmin=yMin; m_desiredYmax=yMax;
	...
	//   m_posY = m_maxY;
	// But account for centering if we have lock aspect:
	m_posX = (m_minX+m_maxX)/2 - ((m_scrX - m_marginLeft - m_marginRight)/2 + m_marginLeft)/m_scaleX ; // m_posX = (xMin+xMax)/2 - (m_scrX/2)/m_scaleX;
//	m_posY = (yMin+yMax)/2 + ((m_scrY - m_marginTop - m_marginBottom)/2 - m_marginTop)/m_scaleY;  // m_posY = (yMin+yMax)/2 + (m_scrY/2)/m_scaleY;
	m_posY = (yMin+yMax)/2 + ((m_scrY - m_marginTop - m_marginBottom)/2 + m_marginTop)/m_scaleY;  // m_posY = (yMin+yMax)/2 + (m_scrY/2)/m_scaleY;
